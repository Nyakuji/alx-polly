...
export async function getPollResults(pollId: string) {
  const { data, error } = await supabase.from('polls').select('options').eq('id', pollId).single();

  if (error) {
    throw new Error(error.message);
  }

  const { data: votesData, error: votesError } = await supabase.from('votes').select('option').eq('poll_id', pollId);
